...
static err_t
echo_server_accept(void *arg, struct tcp_pcb *pcb, err_t err);
static err_t
echo_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err);
static void
echo_server_err(void *arg, err_t err);
static err_t
echo_server_sent(void *arg, struct tcp_pcb *pcb, u16_t len);
static int
echo_server_send(struct tcp_pcb *pcb, const char *buf, size_t len);
